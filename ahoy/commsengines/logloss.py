...
class LogLossComms(CommsEngine) :
    ...
    def _on_send(self, event) :
        source_agent_uid = event.get_src_agent_uid()
        dest_agent_uids = event.get_message().get_dest_agents()

        source_node_uid = self.get_node_from_agent(source_agent_uid)
        dest_node_uids = set([])
        for agent_uid in dest_agent_uids :
            dest_node_uids.add(self.get_node_from_agent(agent_uid))

        valid_dests = set([])

        for dest_node_uid in dest_node_uids :
            ...
            if self._should_deliver(source_node_uid, dest_node_uid, src_power, dest_power) :
                valid_dests.add(dest_node_uid)

        if len(valid_dests) > 0 :
            valid_dests = list(valid_dests)
            message = event.get_message()
            message.set_dest_agents(valid_dests)
            self.get_event_api().publish(CommunicationRecvEvent(event.get_src_agent_uid(), event.get_src_iface_name(), message, event.get_network()))
