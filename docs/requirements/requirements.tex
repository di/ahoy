...
%STAGE provides the capability to test the veracity of system-level sensors and component-level algorithms in the
%laboratory against virtual operations using data from simulated sensors and nodes. These nodes work with scripted or
%real time data, depending on the user's need. This network and sensor testbed allows users to independently vary its
%topology, application suites, and the environment. It also emulates the communication between these components. A
%visualization tool is provided to the user for demoing and analyzing.
\end{abstract}
...
\begin{itemize*}
  ...
  \item ASRA: %place ASRA reference here%
\end{itemize*}
