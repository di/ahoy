...
\subsubsection{Agent}
{An \texttt{Agent} is an intelligent algorithm that makes decisions based on data it receives from its environment and its beliefs about the current state of the system. Agents are run as threads on distributed nodes. }

\attributes{
    \attribute{\_owner\_node}{\texttt{Node}}{Node that owns the \texttt{Agent}}
    \attribute{\_uid}{int}{Unique identifier.}
    \attribute{\_behaviors}{dict(entity: [\texttt{Condition},\texttt{Action}])}{Behavior map linking events to actions.}
}

\operations{
    \operation{get\_uid()}{None}{UID}{Returns the unique ID.}
    \operation{get\_owner\_node()}{None}{Owner \texttt{Node}}{Returns the node that owns the agent.}
    \operation{start()}{None}{A thread}{Returns the \texttt{Agent} thread.}
    \operation{on\_message(event)}{Event generating a message.}{None}{Abstract class for handling message events.}
    \operation{run()}{None}{None}{Abstract class for starting the \texttt{Agent} thread.}
    \operation{add\_behavior(behavior)}{list([\texttt{Condition},\texttt{Event},\texttt{Action}])}{None}{Adds a behavior tuple to the \texttt{Agent}.}
    \operation{remove\_behavior(behavior)}{list[\texttt{Condition},\texttt{Event},\texttt{Action}]}{None}{Removes the specified behavior tuple.}
    \operation{\_on\_event(event)}{An \texttt{Event}.}{None}{Performs an \texttt{Action} based on the \texttt{Event} and \texttt{Condition} associated with it.}
