...
The simulation is configured within a startup script that initializes the world, entities within that world, agents, and the simulation itself.  All properties of these objects, such as interfaces for nodes or behaviors for agents, are also established within this script.  An example can be found in Figure-\ref{fig:exstartup} below.  It demonstrates a scenario with two nodes, each with a wireless network interface.  Each node is then assigned one agent and added to the \texttt{World}.  After all entities, agents, and world objects are instantiated, the \texttt{Simulation} is created and started.
\begin{figure}
    \begin{verbatim}
import sys
import signal
...
agent1 = CommsAgent(n1,5,6,False)
act1 = MoveAction(n1,1,1,0)
con1 = SourceCondition(6)
agent1.add_behavior([con1,CommunicationSendEvent,act1])
n1.add_agent(agent1)
world.add_entity(n1)
