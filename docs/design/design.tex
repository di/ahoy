...
\begin{figure}
    ...
    \begin{tikzpicture}[node distance=.1cm]
        \node [component] (Entity) {\textbf{Entity}};
        \node [component, right of=Entity] (World) {\textbf{World}};
        \node [component, below of=World] (EventAPI) {\textbf{EventAPI}};
        \node [component, below of=EventAPI] (Network) {\textbf{Network}};
        \node [component, below of=EventAPI, yshift=-4cm] (Event) {\textbf{Event}};
        \node [component, right of=World] (Simulation) {\textbf{Simulation}};
        \node [component, above of=Entity] (Sensor) {\textbf{Sensor}};
        \node [component, below left of=Entity] (Node) {\textbf{Node}};
        \node [component, left of=Entity] (ScriptedEntity) {\textbf{ScriptedEntity}};
        \node [component, below left of=Node] (Agent) {\textbf{Agent}};
        \node [component, below right of=Node] (Interface) {\textbf{Interface}};
        \node [component, below of=Simulation] (CommsEngine) {\textbf{CommsEngine}};
        \node [component, above left of=Agent] (Condition) {\textbf{Condition}};
        \node [component, below left of=Agent] (Action) {\textbf{Action}};

        \node [component, below left of=Event, xshift=-.5cm] (CommunicationSendEvent) {\textbf{CommunicationSendEvent}};
        \node [component, below right of=Event, xshift=.5cm] (CommunicationRecvEvent) {\textbf{CommunicationRecvEvent}};
        \node [component, above right of=Event] (LinkEvent) {\textbf{LinkEvent}};
        \node [component, above left of=Event] (MoveEvent) {\textbf{MoveEvent}};
        \node [component, above of=Event] (StopSimulationEvent) {\textbf{StopSimulationEvent}};
        \node [component, right of=Event] (StartupEvent) {\textbf{StartupEvent}};
        \node [component, left of=Event] (StartSimulationEvent) {\textbf{StartSimulationEvent}};
        \node [component, below of=Event] (AckStartupEvent) {\textbf{AckStartupEvent}};

        \path (CommunicationSendEvent) edge[->, >=open triangle 45] node {} (Event);
        \path (CommunicationRecvEvent) edge[->, >=open triangle 45] node {} (Event);
        \path (LinkEvent) edge[->, >=open triangle 45] node {} (Event);
        \path (MoveEvent) edge[->, >=open triangle 45] node {} (Event);
        \path (StopSimulationEvent) edge[->, >=open triangle 45] node {} (Event);
        \path (StartupEvent) edge[->, >=open triangle 45] node {} (Event);
        \path (StartSimulationEvent) edge[->, >=open triangle 45] node {} (Event);
        \path (AckStartupEvent) edge[->, >=open triangle 45] node {} (Event);
...
This class is used by the end user to begin a simulation.  For example, the script in Figure~\ref{fig:exstartup} creates a scenario with five nodes, each with one \texttt{CustomAgent}.  Running this file will begin a scenario after waiting five seconds for \texttt{StartupDaemon}s to respond.
