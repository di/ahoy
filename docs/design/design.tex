...
\begin{center}
    \begin{longtable}{|p{3cm}|p{9cm + 2.0\tabcolsep}|}
    \hline
...
\attributes{
    ...
    \attribute{\_comms\_module}{CommsEngine}{Communication Engine used to send and receive events}
    \attribute{\_world}{World}{Instance of the world maintained by the simulation}
    \attribute{\_tcp\_forwarder}{TcpForward}{TcpForward object that maintains a port to listen on for packets of data }
}


\operations{
    \operation{\_\_init\_\_(world\_inst, comms\_module, tcp\_forwarder=None)}
    {
        \begin{itemize*}
            \item \texttt{world\_inst}: Instance of the world. 
            \item \texttt{comms\_module}: Communication Engine for sending/receiving events. 
            \item \texttt{tcp\_forwarder}: TcpForward object, if needed.
        \end{itemize*}
    }{None}{Instantiates the Simulation}
    \operation{get\_world()}{None}{World}{Gets the World object being maintained}
    \operation{get\_on\_ack\_startup(event)}
    {
        \begin{itemize*}
            \item \texttt{event}: Event containing the startup daemon id.
        \end{itemize*}
    }    {None}{Adds a startup daemon id to the \_startup\_acks list}
    \operation{start(wait)}
    {
        \begin{itemize*}
            \item \texttt{wait}: Number of seconds to wait before starting.
        \end{itemize*}
    }{None}{Starts the Simulation. Startup daemons are alerted and the World and the entities within it are initialized}
...
\operations{
    \operation{\_\_init\_\_(phys\_id)}
    {
        \begin{itemize*}
            \item \texttt{phys\_id}: The integer id of the daemon.
        \end{itemize*}
    }{None}{Instantiates the StartupDaemon.}
    \operation{\_on\_startup(event)}
    {
        \begin{itemize*}
            \item \texttt{event}: Event passed to the daemon on startup. 
        \end{itemize*}
    }{None}{Subscribes the daemon to the StartSimulationEvent and publishes an AckStartupEvent.  It also initializes the \_world attribute. }
    \operation{terminate\_all()}{None}{None}{Kills all processes maintained in its list \_running\_pids}
    \operation{\_start\_entity\_processes(entity)}
    {
        \begin{itemize*}
            \item \texttt{entity}: Entity to start and maintain.
        \end{itemize*}
    }{None}{Starts the entity passed in and adds its process id to the list \_running\_pids}
    \operation{\_on\_sim\_start(event)}
    {
        \begin{itemize*}
            \item \texttt{event}: StartSimulationEvent published at the start of the simulation. 
        \end{itemize*}
    }{None}{Grabs the list of entities to maintain from event and calls \_start\_entity\_process with each one}
    \operation{start()}{None}{None}{Starts the daemon. Subscribes to StartupEvent and StopSimulationEvent events}
    \operation{\_restart(event)}
    {
        \begin{itemize*}
            \item \texttt{event}: Event that triggered the call to \_restart.
        \end{itemize*}
    }{None}{Resets the state of the daemon. All processes maintained are cleared.}
